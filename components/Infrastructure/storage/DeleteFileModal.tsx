...
export const DeleteFileModal: React.FC<DeleteFileModalProps> = ({ onClose, onConfirm, count, fileName }) => {
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-neutral-900 w-full max-w-md rounded-3xl shadow-2xl p-8 border border-gray-200 dark:border-neutral-700">
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Delete File{count > 1 ? 's' : ''}?</h3>
          
          <p className="text-gray-500 dark:text-gray-400 leading-relaxed">
            Are you sure you want to permanently delete {count > 1 ? <span className="font-bold text-gray-900 dark:text-white">{count} selected files</span> : <span className="font-bold text-gray-900 dark:text-white">{fileName}</span>}? 
