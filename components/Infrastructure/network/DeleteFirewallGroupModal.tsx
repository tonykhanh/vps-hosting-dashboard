...
interface DeleteFirewallGroupModalProps {
  groupName: string;
  onClose: () => void;
  onConfirm: () => void;
}

export const DeleteFirewallGroupModal: React.FC<DeleteFirewallGroupModalProps> = ({ groupName, onClose, onConfirm }) => {
  const [confirmName, setConfirmName] = useState('');
  const isMatch = confirmName === groupName;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-neutral-900 w-full max-w-md rounded-3xl shadow-2xl p-8 border border-gray-200 dark:border-neutral-700">
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Delete Firewall Group?</h3>
          
          <p className="text-gray-500 dark:text-gray-400 leading-relaxed text-sm">
            This will permanently delete <span className="font-bold text-gray-900 dark:text-white">{groupName}</span>. 
            Any instances currently using this group will lose these rules immediately, which may disrupt connectivity.
          </p>
          ...
          <div className="w-full text-left bg-gray-50 dark:bg-neutral-800 p-4 rounded-xl border border-gray-200 dark:border-neutral-700 mt-2">
             <label className="block text-xs font-bold text-gray-500 mb-2 uppercase">Type <span className="select-all text-gray-900 dark:text-white">{groupName}</span> to confirm</label>
             <input 
                type="text" 
