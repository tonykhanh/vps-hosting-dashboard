...
interface AttachIpModalProps {
  ipAddress: string;
  onClose: () => void;
  onAttach: (instanceName: string) => void;
}

export const AttachIpModal: React.FC<AttachIpModalProps> = ({ ipAddress, onClose, onAttach }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white dark:bg-neutral-900 w-full max-w-md rounded-3xl shadow-2xl p-6 border border-gray-200 dark:border-neutral-700" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <Link2 size={20} className="text-plasma-600"/> Attach IP Address
          </h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><X size={24}/></button>
        </div>

        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          Select an instance to attach the reserved IP <strong className="text-gray-900 dark:text-white font-mono">{ipAddress}</strong> to.
        </p>
        ...
        <div className="mt-8 flex gap-3">
          ...
          <Button onClick={handleSubmit} className="flex-1 shadow-lg shadow-plasma-500/20">Attach IP</Button>
        </div>
      </div>
    </div>,
    document.body
