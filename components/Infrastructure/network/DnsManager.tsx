...
import { DOMAIN_LIST, DNS_RECORDS } from '../../../constants';
...
export const DnsManager: React.FC = () => {
  ...
  const handleAddDomain = () => {
    ...
    // @ts-ignore - simplified type matching for mock
  ...
  const handleSaveRecord = () => {
    if (editingRecord) {
      setRecords(prev => prev.map(r => r.id === editingRecord.id ? { ...r, ...recordForm } : r));
    } else {
      const newRecord = {
        id: `r-${Date.now()}`,
        ...recordForm
