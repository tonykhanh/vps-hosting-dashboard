...
import { 
  ...
  Cpu, Server, RefreshCw, Undo2, Power, Play
} from 'lucide-react';
import { Button } from './Button';

export type ActionType = 'DEPLOY' | 'REBOOT' | 'SCALE' | 'ROLLBACK' | 'STOP' | 'START' | null;
...
const ACTION_CONFIGS: Record<string, { title: string, icon: any, steps: Step[] }> = {
  ...
  STOP: {
    title: "Suspend Capsule",
    icon: Power,
    steps: [
      { id: '1', label: 'Traffic Drain', status: 'pending', detail: 'Closing active sessions' },
      { id: '2', label: 'Unmount Storage', status: 'pending', detail: 'Safely detaching volumes' },
      { id: '3', label: 'Halt Container', status: 'pending', detail: 'Stopping runtime' },
      { id: '4', label: 'Release Resources', status: 'pending', detail: 'Freeing compute allocation' },
    ]
  },
  START: {
    ...
    icon: Play,
    steps: [
      { id: '1', label: 'Provision Compute', status: 'pending', detail: 'Allocating host resources' },
      { id: '2', label: 'Attach Storage', status: 'pending', detail: 'Mounting persistent volumes' },
      { id: '3', label: 'Boot Runtime', status: 'pending', detail: 'Starting container services' },
      { id: '4', label: 'Health Check', status: 'pending', detail: 'Verifying endpoints' },
    ]
  }
};

export const ActionFlow: React.FC<ActionFlowProps> = ({ isOpen, type, onClose, onStart, onComplete, projectName }) => {
    ...
    const interval = window.setInterval(() => {
      setSteps(prev => {
        ...
        if (idx + 1 < newSteps.length) {
          if (newSteps[idx + 1]) {
            newSteps[idx + 1] = { ...newSteps[idx + 1], status: 'running' };
          }
        }
        return newSteps;
      });
      ...
      if (idx >= steps.length) {
        ...
        if (onComplete) {
          setTimeout(onComplete, 800); // Small delay before applying effect
        }
      }
    }, 1500); // Slower simulation for better visibility
  ...
  if (!isOpen || !type || !ACTION_CONFIGS[type]) return null;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-end">
      {/* Backdrop - blocks clicks when running */}
      <div 
        className={`absolute inset-0 bg-neutral-900/30 backdrop-blur-sm transition-opacity ${flowState === 'running' ? 'cursor-wait' : 'cursor-pointer'}`} 
        onClick={flowState === 'running' ? undefined : onClose}
      />
      ...
      <div className="relative w-full md:max-w-lg h-full bg-white/90 dark:bg-neutral-900/95 backdrop-blur-2xl border-l border-white/50 dark:border-white/10 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="p-8 border-b border-gray-100 dark:border-white/5 bg-gradient-to-br from-white to-gray-50 dark:from-neutral-900 dark:to-neutral-800">
          <div className="flex justify-between items-start mb-6">
            ...
            {/* Close Button - Disabled during flow */}
            <button 
              onClick={onClose} 
          ...
          <div className={`mt-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border ${
            ...
            'bg-gray-100 dark:bg-neutral-800 text-gray-700 dark:text-gray-300'
             ...
             <span className="uppercase tracking-wide">
               {flowState === 'idle' ? 'Ready to Start' : flowState === 'running' ? 'Executing Graph...' : 'Complete'}
             </span>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-gray-100 dark:border-white/5 bg-white dark:bg-neutral-900">
          {flowState === 'idle' && (
            <div className="flex gap-3">
              ...
              <Button onClick={startFlow} className="flex-[2] shadow-lg shadow-plasma-500/20" disabled={steps.length === 0}>
          ...
          {flowState === 'running' && (
             <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-lg p-3 text-amber-800 dark:text-amber-200 text-sm flex items-center justify-center animate-pulse">
                <span className="flex items-center gap-2 font-bold">
                   <Loader2 size={16} className="animate-spin" /> Action in progress. Do not close.
                </span>
             </div>
          )}
